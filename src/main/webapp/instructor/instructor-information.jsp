...
<%
  Employee instructor = (Employee) request.getAttribute("instructor");
%>
  ...
  <main class="main">
    ...
    <section class="question-mana">
      <header class="question-mana__header">
        <h2 class="heading-secondary">Thông tin Giảng viên</h2>
      </header>
      
      <div class="info grid grid--2-cols">
        <article class="info__article">
          <div><i class='bx bx-check-shield'></i>
          <p>Mã số Giảng viên: <%= instructor.getEmployeeID() %></p>
          </div>
          <div><i class='bx bx-check-shield'></i>
          <p>Họ tên: <%= instructor.getLastName() + " " + instructor.getFirstName() %></p>
          </div>
          <div><i class='bx bx-check-shield'></i>
          <p>Thuộc khoa: <%= instructor.MajorName() %></p>
          </div>
          <div><i class='bx bx-check-shield'></i>
          <p>Giới tính: <%= instructor.getGender() ? "Nam" : "Nữ" %></p>
          </div>
          <div><i class='bx bx-check-shield'></i>
          <p>Ngày sinh: <%= instructor.getDateOfBirth() %></p>
          </div>
          <div><i class='bx bx-check-shield'></i>
          <p>Số điện thoại: <%= instructor.getPhone() %></p>
          </div>
          <div><i class='bx bx-check-shield'></i>
          <p>Email: <%= instructor.getEmail() %></p>
          </div>
        </article>
        
        <figure class="info__figure">
          <img src="../assets/img/lecture.jpg" alt="anh giang vien">
          <figcaption>
            Giảng viên <%= instructor.getLastName() + " " + instructor.getFirstName() %>
          </figcaption>
        </figure>
      </div>
    </section>
